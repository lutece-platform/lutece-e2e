...
/**
 * Brique macro : ajouter une question de type {@code DATE} (date).
 *
 * <p>Type de base : l'ajout DOIT reussir. Delegue au moteur {@link AddQuestionMacroTest}.</p>
 */
@Epic("Forms")
@Feature("Questions")
@Story("Ajouter une question date")
@Tag("macro")
@Tag("forms")
@Tag("brick")
public class AddQuestionDateMacroTest extends MacroTest {

    private static final QuestionType TYPE = QuestionType.DATE;
    private static final String TITLE = "Question date";

    @Step("Ajouter une question date")
    public static void run(FormsContext ctx) {
        AddQuestionMacroTest.run(ctx, QuestionDataSet.of(TYPE, TITLE));
    }

    @Test
    @DisplayName("Ajouter une question date (auto-provisionnement formulaire + etape)")
    void standalone() {
