...
/**
 * Brique macro : ajouter une question de type {@code TEXTAREA} (texte long).
 *
 * <p>Type de base : l'ajout DOIT reussir. Delegue au moteur {@link AddQuestionMacroTest}.</p>
 */
@Epic("Forms")
@Feature("Questions")
@Story("Ajouter une question texte long")
@Tag("macro")
@Tag("forms")
@Tag("brick")
public class AddQuestionTextareaMacroTest extends MacroTest {

    private static final QuestionType TYPE = QuestionType.TEXTAREA;
    private static final String TITLE = "Question texte long";

    @Step("Ajouter une question texte long")
    public static void run(FormsContext ctx) {
        AddQuestionMacroTest.run(ctx, QuestionDataSet.of(TYPE, TITLE));
    }

    @Test
    @DisplayName("Ajouter une question texte long (auto-provisionnement formulaire + etape)")
    void standalone() {
