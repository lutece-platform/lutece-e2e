...
/**
 * Brique macro : ajouter une question de type {@code TEXT} (texte court).
 *
 * <p>Type de base : l'ajout DOIT reussir. Delegue au moteur {@link AddQuestionMacroTest}.</p>
 */
@Epic("Forms")
@Feature("Questions")
@Story("Ajouter une question texte court")
@Tag("macro")
@Tag("forms")
@Tag("brick")
public class AddQuestionTextMacroTest extends MacroTest {

    private static final QuestionType TYPE = QuestionType.TEXT;
    private static final String TITLE = "Question texte court";

    @Step("Ajouter une question texte court")
    public static void run(FormsContext ctx) {
        AddQuestionMacroTest.run(ctx, QuestionDataSet.of(TYPE, TITLE));
    }

    @Test
    @DisplayName("Ajouter une question texte court (auto-provisionnement formulaire + etape)")
    void standalone() {
