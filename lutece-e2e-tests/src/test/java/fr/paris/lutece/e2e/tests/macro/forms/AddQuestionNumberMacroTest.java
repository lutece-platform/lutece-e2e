...
/**
 * Brique macro : ajouter une question de type {@code NUMBER} (nombre).
 *
 * <p>Type de base : l'ajout DOIT reussir. Delegue au moteur {@link AddQuestionMacroTest}.</p>
 */
@Epic("Forms")
@Feature("Questions")
@Story("Ajouter une question nombre")
@Tag("macro")
@Tag("forms")
@Tag("brick")
public class AddQuestionNumberMacroTest extends MacroTest {

    private static final QuestionType TYPE = QuestionType.NUMBER;
    private static final String TITLE = "Question nombre";

    @Step("Ajouter une question nombre")
    public static void run(FormsContext ctx) {
        AddQuestionMacroTest.run(ctx, QuestionDataSet.of(TYPE, TITLE));
    }

    @Test
    @DisplayName("Ajouter une question nombre (auto-provisionnement formulaire + etape)")
    void standalone() {
